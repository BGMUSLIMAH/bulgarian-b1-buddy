...
import { LevelUpCelebration, getLevelMeta } from "@/components/LevelUpCelebration";
...
function ProgressPage() {
  ...
  const latestEval = stats.evaluations[0] ?? null;

  // Streak warning: lastPracticeDate is yesterday → at risk
  const streakAtRisk = (() => {
    if (!stats.lastPracticeDate || stats.streakDays === 0) return false;
  ...
  return (
    <>
      {/* 🎉 Celebration overlay — fires automatically on level-up */}
      <LevelUpCelebration currentLevel={lvl.level} />
      ...
      <div className="space-y-8">
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {/* XP / Level card — now uses level color + emoji */}
          <div
            className="md:col-span-2 rounded-2xl border p-6"
            style={{
              background: `linear-gradient(135deg, hsl(var(--card)) 0%, ${meta.color}18 100%)`,
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-0.5 font-display text-3xl font-bold" style={{ color: meta.color }}>
                    {lvl.title}
                  </h2>
                  <p className="text-xs text-muted-foreground italic">{meta.subtitle}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-xs text-muted-foreground">XP</p>
                <p className="font-display text-3xl font-bold" style={{ color: meta.color }}>{stats.xp}</p>
              </div>
            </div>
            <div className="mt-5">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {lvl.current} / {lvl.next} XP → Level {lvl.level + 1}
                {lvl.level < 20 && (
                  <span className="ml-2 opacity-60">({getLevelMeta(lvl.level + 1).emoji} {getLevelMeta(lvl.level + 1).title})</span>
                )}
              </p>
            </div>
          </div>
          ...
          {/* Streak card */}
          <div className={`rounded-2xl border p-6 ${streakAtRisk ? "border-yellow-500/50 bg-yellow-500/5" : "border-border bg-card"}`}>
            ...
            <p className="mt-1 font-display text-5xl font-bold">
              {streakAtRisk ? "⚠️" : "🔥"} <span className="text-foreground">{stats.streakDays}</span>
            </p>
            <p className="mt-2 text-xs text-muted-foreground">
              {stats.streakDays === 0
                ? "Practice today to start a streak."
                : streakAtRisk
                ? "Practice today or your streak resets!"
                : stats.streakDays === 1
        ...
        {/* CEFR badge — only shown if evaluation exists */}
        {latestEval && (
          <section className="flex items-center gap-4 rounded-2xl border border-border bg-card px-6 py-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-primary/10 font-display text-2xl font-bold text-primary">
              {latestEval.level}
            </div>
            <div>
              <p className="text-xs uppercase tracking-wider text-muted-foreground">Latest CEFR evaluation</p>
              <p className="font-medium">{latestEval.score}/{latestEval.total} correct — {Math.round((latestEval.score / latestEval.total) * 100)}%</p>
              <p className="text-xs text-muted-foreground">{new Date(latestEval.date).toLocaleDateString()}</p>
            </div>
            <div className="ml-auto text-right">
              <p className="text-xs text-muted-foreground">Take again?</p>
              <a href="/evaluation" className="text-xs font-medium text-primary hover:underline">Start test →</a>
            </div>
          </section>
        )}
